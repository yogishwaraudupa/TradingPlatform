...
export default function LiveData({ onClose, onSelect }){
  ...
  useEffect(()=>{
    fetchData()
    const id = setInterval(fetchData, 3000) // concurrent poll 3s
    return ()=>clearInterval(id)
  },[])

  const filtered = filter==='all' ? data : data.filter(d=>d.assetClass===filter)
  const stats = {
    total: data.length,
      ...
      <div onClick={e=>e.stopPropagation()} style={{maxWidth:1200, margin:'10px auto', background:'#1e2329', border:'1px solid #2b3139', borderRadius:16, overflow:'hidden'}}>
        <div style={{padding:'12px 16px', display:'flex', justifyContent:'space-between', alignItems:'center', borderBottom:'1px solid #2b3139', flexWrap:'wrap', gap:8}}>
          <div style={{display:'flex', alignItems:'center', gap:10}}>
            ...
            <span style={{fontSize:11, opacity:0.6}}>Last tick: {lastTick} • Auto 3s</span>
          </div>
          <button className="btn btn-ghost" onClick={onClose}>✕ Close</button>
        </div>

        <div style={{display:'flex', gap:6, padding:'10px 12px', flexWrap:'wrap', background:'#0b0e11', borderBottom:'1px solid #2b3139'}}>
          {['all','stocks','crypto','forex','commodity','index'].map(f=>(
            <button key={f} onClick={()=>setFilter(f)} className={`tab ${filter===f?'active':''}`} style={{padding:'6px 10px', fontSize:11}}>{f.toUpperCase()} ({f==='all'?data.length: data.filter(d=>d.assetClass===f).length})</button>
          ))}
          <span style={{marginLeft:'auto', fontSize:11, opacity:0.6, alignSelf:'center'}}>Click any row → open chart</span>
        </div>

        <div style={{overflow:'auto', maxHeight:'65vh'}}>
          <table className="table" style={{minWidth:900}}>
            <thead style={{position:'sticky', top:0, background:'#1e2329', zIndex:1}}>
              <tr><th>#</th><th>SYMBOL</th><th>CLASS</th><th>PRICE</th><th>CHANGE</th><th>%</th><th>PREV CLOSE</th><th>SOURCE</th><th>LAST UPDATE</th><th>ACTION</th></tr>
            </thead>
            <tbody>
              {filtered.map((d,i)=>(
              ...
              {filtered.length===0 && <tr><td colSpan={10} style={{textAlign:'center', padding:20, opacity:0.6}}>No data for {filter}</td></tr>}
            </tbody>
          </table>
        </div>
        ...
        <div style={{padding:'8px 12px', fontSize:11, opacity:0.6, borderTop:'1px solid #2b3139', display:'flex', justifyContent:'space-between', flexWrap:'wrap', gap:8}}>
          <span>Concurrent polling 3s • Yahoo Chart + CoinGecko • 20 symbols</span>
          <span>Tip: Click ticker or table row to open TradingView chart instantly</span>
        </div>
      </div>
